...
All public Objective-C classes, protocols, enums, typedefs, and constants from CoreLocation framework headers (MacOSX26.2.sdk) were enumerated by scanning the `Headers/` directory. Symbols marked `API_UNAVAILABLE(macos)` or `API_DEPRECATED` for macOS 26.0 are classified as EXEMPT with their SDK attributes documented. The crate's Rust API and Swift bridge thunks were cross-referenced against SDK symbols to classify each top-level symbol as VERIFIED or EXEMPT. No new symbols were discovered in 26.2 relative to the v1 audit baseline.
